...
public class YoutubeSoundtrackScrapper {
    ...
    public static String toYoutubeUrl(String hash){
        return YOUTUBE_WATCH_URL_PREFIX +hash;
    }
    public static String hashFromYoutubeUrl(String url){
        int index=url.indexOf(YOUTUBE_HASH_PREFIX);
        if(index!=-1) {
            return url.substring(index+YOUTUBE_HASH_PREFIX.length()).replace("/","").replace("\\","").trim();
        }
        return null;
    }
