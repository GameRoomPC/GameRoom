...
public class YoutubeSoundtrackScrapper {
    ...
    private static String YOUTUBE_WATCH_URL_HASH_PREFIX = "https://www.youtube.com/watch?v=";
    ...
    public static String hashFromYoutubeUrl(String url){
        int index=url.indexOf(YOUTUBE_WATCH_URL_HASH_PREFIX)+YOUTUBE_WATCH_URL_PREFIX.length();
        if(index!=-1) {
            return url.substring(index).replace("/","").replace("\\","").trim();
        }
        return null;
    }
